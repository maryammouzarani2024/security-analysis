...
def dirty_patch_size(query, max_size, signed=False):
    ...
    if max_size == query.size:
        ...
        if isinstance(query, STP_function) and query.op in [ '=' ]:
            return query
        assert (isinstance(query, STP_variable) or query.op in [ 'IF', '@', 'BVPLUS', '~', 'BVSX', '&', '|', '>>', 'BVMULT', 'BVSUB',  'BVXOR' ]), 'Failed:\n%s' % query.pp()
        ...
        if isinstance(query, STP_function):
            ...
            # (0h000000@(NOT(x1 = 0h62))) is invalid
                ...
                assert query.op in [ 'IF', '@', 'BVPLUS', '~', 'BVSX', '&', '|', '>>', 'BVMULT', 'BVSUB' ,  'SBVDIV'] or query.op[0] == '[', query.op
                if not signed:
...
class STP:
    ...
    def execute2(self):
        ...
        fp.write(self.pp())
        fp.close()
    def bof_query(selfself, maxlen, min, max):
        destination= open( "/tmp/stp_input.cvc", "w" )
        source= open( "/tmp/stp_input.txt", "r" )
        x=0
        y=0
        z=0
        for line in source:
            if  line.find("BITVECTOR") >0 and y!=-1:
                m=re.match("x(\d+).*", line)
                if m:
                    s=int(m.group(1))
                    if int(m.group(1))>x:
                        x=int(m.group(1))
#                m=re.match("x(\d\d).*", line)
#                if m:
#                    s=int(m.group(1))
#                    if int(m.group(1))>x:
#                        x=int(m.group(1))
#                m=re.match("x(\d\d\d).*", line)
#                if m:
#                    s=int(m.group(1))
#                    if int(m.group(1))>x:
#                        x=int(m.group(1))
#                m=re.match("x(\d\d\d\d).*", line)
#                if m:
#                    s=int(m.group(1))
#                    if int(m.group(1))>x:
#                        x=int(m.group(1))
                ...
                len=x+maxlen+2# +10 to make sure of the overflow
                y=-1
                if z==0:
                    destination.write( "bv0 : BITVECTOR("+str((len+1)*8)+");\n" )
                    z=1
                var_cnt=x
                if line.find("QUERY")>=0:
                        while var_cnt>max and var_cnt<=x:
                            replace_str="bv0["+str((var_cnt+maxlen+1)*8-1)+":"+str((var_cnt+maxlen)*8)+"]"
                            search_str="x"+str(var_cnt)
                            line = line.replace(search_str, replace_str)
                            var_cnt=var_cnt-1
                        while var_cnt<=max and var_cnt>-1:
                            replace_str="bv0["+str((var_cnt+1)*8-1)+":"+str(var_cnt*8)+"]"
                            search_str="x"+str(var_cnt)
                            line = line.replace(search_str, replace_str)
                            var_cnt=var_cnt-1
    ...
    def execute(self):
       ...
       #     sys.exit(-1)
            ...
            self.result = stdout
    def execute3(self):
        '''Execute stp, giving in input a STP QUERY FOR BOF'''
        myq=""
        fp2 = open('/tmp/stp_input.cvc', 'r')
        for line in fp2:
            myq += line+ '\n'
        input_file="/tmp/stp_input.cvc"
        stdin = open(input_file, 'r')
        p = subprocess.Popen([ STP_PROG, '-p', ], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        (stdout, stderr) = p.communicate(myq)
        if stderr:
            print 'STP.execute() failed on :'
            print '-' * 80
            print self.pp(),
    ...
    def interpret2(self):
        ...
        if lines[0] == 'Valid.':
            ...
            for l in lines[1:-1]:
                l = re.sub('\s*', '', l)
                m = re.match('ASSERT\(bv0=0hex([0-9a-zA-Z]+)\)', l)
                if m:
                    value = m.group(1)
                
                else:
                    m = re.match('ASSERT\(v_solver_0=0hex([0-9a-zA-Z]+)\)', l)
                    if m:
                        value = m.group(1)
                    else:
                        print 'STP.interpret(%s) failed' % l
            ...
            for i in range(0, len(value), 2):
                        byte_number =z
                        solution.append(int(value[i:i+2], 16))
                        z=z+1
            return solution
