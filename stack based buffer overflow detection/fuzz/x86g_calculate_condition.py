...
def sub(size, cond, cc_dep1, cc_dep2):
    ...
    #khodam
        ...
        if   size == 8:  return '1Uto32(CmpLT8U(32to8(%s),32to8(%s)))' % (cc_dep1, cc_dep2)
        elif size == 32: return '1Uto32(CmpLT32U(%s,%s))' % (cc_dep1, cc_dep2)
        
    assert False, "Can't generate constraint for cond=%s, size=%d" % (cond, size)

def add(size, cond, cc_dep1, cc_dep2):
    if cond == 'X86CondZ':
        if   size == 8:  return '1Uto32(CmpEQ8(Add8(32to8(%s),32to8(%s)),0x00))' % (cc_dep1, cc_dep2)
        elif size == 32: return '1Uto32(CmpEQ32(Add32(%s,%s),0x00))' % (cc_dep1, cc_dep2)
    elif cond == 'X86CondLE':
        if   size == 32: return '1Uto32(CmpLE32S(%s,%s))' % (cc_dep1, cc_dep2)
    elif cond == 'X86CondL':
        if   size == 32: return '1Uto32(CmpLT32S(%s,%s))' % (cc_dep1, cc_dep2)
...
def logic(size, cond, cc_dep1, cc_dep2):
    if cond == 'X86CondZ':
        if   size == 8:  return '1Uto32(CmpEQ8(32to8(%s),32to8(%s)))' % (cc_dep1, cc_dep2)
        elif size == 32: return '1Uto32(CmpEQ32(%s,%s))' % (cc_dep1, cc_dep2)
    elif cond == 'X86CondLE':
...
def inc(size, cond, cc_dep1, cc_dep2):
    if cond == 'X86CondZ':
        if   size == 8:  return '1Uto32(CmpEQ8(32to8(%s),32to8(%s)))' % (cc_dep1, cc_dep2)
        elif size == 32: return '1Uto32(CmpEQ32(%s,%s))' % (cc_dep1, cc_dep2)
    elif cond == 'X86CondLE':
...
def dec(size, cond, cc_dep1, cc_dep2):
    if cond == 'X86CondZ':
        if   size == 8:  return '1Uto32(CmpEQ8(32to8(%s),32to8(%s)))' % (cc_dep1, cc_dep2)
        elif size == 32: return '1Uto32(CmpEQ32(%s,%s))' % (cc_dep1, cc_dep2)
    elif cond == 'X86CondLE':
...
def x86g_calculate_condition(cond, cc_op, cc_dep1, cc_dep2):
    ...
    size = SIZE[cc_op[-1]]
    #khodam movaghatan be khatere khata size ra 32 kardam baraye test sql_good
    #size=32
    if ('SUB' in cc_op) :
        e = sub(size, cond, cc_dep1, cc_dep2)
    elif ('ADD' in cc_op) :
        e = add(size, cond, cc_dep1, cc_dep2)
    elif 'SUBB' in cc_op:
        e = sub(size, cond, cc_dep1, cc_dep2)
    elif 'LOGIC' in cc_op:
        e=logic(size, cond, cc_dep1, cc_dep2)
    elif 'INC' in cc_op:
        e=inc(size, cond, cc_dep1, cc_dep2)
    elif 'DEC' in cc_op:
        e=dec(size, cond, cc_dep1, cc_dep2)    
    else:
        assert False, cc_op
