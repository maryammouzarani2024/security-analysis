...
#                except:
#                    print 'errrrrrrrrrrrr',n.expr[1].pp(),  n.expr[2].pp(),  n.expr[3].pp()
#                    return 
            else:
...
def run_valgrind(progname, progarg, input_file, taint_stdin=False, max_constraint=-1):
    ...
    #taint_stdin=True 
    if not taint_stdin:
        arg_valgrind = [ FUZZGRIND,'--vex-iropt-level=0','--vex-guest-chase-thresh=0', '--tool=fuzzgrind', '--taint-file=yes', '--file-filter=' + input_file ]
        #arg_valgrind = [ FUZZGRIND,'--vex-guest-chase-thresh=0','--vex-iropt-level=0','--tool=fuzzgrind', '--taint-file=yes', '--file-filter=' + input_file ]
        #arg_valgrind = [ FUZZGRIND,'--vex-iropt-level=0','--show-below-main=yes', '--vex-guest-chase-thresh=0','--trace-flags=10000000','--trace-notbelow=1518','--trace-children=yes','--vex-iropt-register-updates=allregs-at-each-insn', '--tool=fuzzgrind', '--taint-file=yes', '--file-filter=' + input_file ]
        stdin = None
    else:
